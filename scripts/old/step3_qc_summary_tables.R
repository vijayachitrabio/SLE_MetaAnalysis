...
rep_dt   <- fread(file.path(RESULTS_DIR, "finngen_replication_v2_results.tsv"))
...
inflation_summary <- data.table(
  ...
  Lambda  = c(1.409, 1.059, 0.917),
  GC_Applied = c("Yes","Yes","N/A (GC applied to inputs)")
)
...
top_table <- merge(top_table,
                   rep_dt[, .(SNP, BETA_fg, P_fg, replicated)], by="SNP", all.x=TRUE)
top_table <- merge(top_table,
...
supp_qc <- data.table(
  ...
  Lambda_raw      = c(1.409, 1.059, NA),
  GC_Corrected    = c("Yes","Yes","N/A"),
  Lambda_post_GC  = c(1.000, 1.000, 0.917),
